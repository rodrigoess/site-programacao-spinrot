  ...
  rarityOrder.forEach(rarity => {
    if (groupedItems[rarity]) {
      ...
      groupedItems[rarity].forEach((item, index) => {
        ...
        selectBtn.addEventListener('click', () => selectItemForTrade(item, inventory.indexOf(item)));
...
function selectItemForTrade(item, index) {
  if (selectedItems.some(selected => selected.index === index)) {
    // Deselect item
    selectedItems = selectedItems.filter(selected => selected.index !== index);
  } else {
    // Select item
...
function updateSelectedItemsDisplay() {
  ...
  selectedItems.forEach(item => {
    ...
    removeBtn.addEventListener('click', () => {
      selectedItems = selectedItems.filter(selected => selected.index !== item.index);
      updateSelectedItemsDisplay();
      updateProposeTradeButton();
    });
  ...
  if (proposeBtn) {
    proposeBtn.addEventListener('click', () => {
      if (selectedItems.length > 0) {
        ...
        document.getElementById('trade-details').textContent = `Você está oferecendo: ${tradeDetails}`;
