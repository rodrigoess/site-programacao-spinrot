...
function displayMarketListings() {
  ...
  marketListings.forEach((listing, index) => {
    ...
    listingCard.innerHTML = `
      <img src="https://via.placeholder.com/100x100?text=${encodeURIComponent(listing.item.item)}" alt="${listing.item.item}" />
      <div class="item-info">
        <h5>${listing.item.item}</h5>
        <p class="rarity-label ${listing.item.rarity}">${listing.item.rarity.charAt(0).toUpperCase() + listing.item.rarity.slice(1)}</p>
        <p class="item-price"><i class="ri-coin-fill"></i> ${listing.price} Moedas</p>
      </div>
    ...
    if (groupedItems[rarity]) {
      ...
      groupedItems[rarity].forEach((item, index) => {
        ...
        itemCard.innerHTML = `
          ...
          <button class="btn btn-sm btn-outline-primary sell-item-btn" data-index="${inventory.indexOf(item)}">Vender</button>
        `;

        // Add click event to sell button
        const sellBtn = itemCard.querySelector('.sell-item-btn');
        sellBtn.addEventListener('click', () => showSellModal(item, inventory.indexOf(item)));
...
function getSuggestedPrice(rarity) {
  const prices = {
    rare: 15,      // Higher than box price (box rare costs ~5-10)
    epic: 30,      // Higher than roulette epic
    legendary: 50, // Higher than roulette legendary
    mythical: 100, // Much higher for rare items
    secret: 200    // Very high for secret items
  };
...
function showPurchaseModal(listing, index) {
  document.getElementById('purchase-details').textContent =
    `Comprar "${listing.item.item}" por ${listing.price} moedas?`;
...
function showSellModal(item, index) {
  document.getElementById('sell-item-details').textContent = `Vender "${item.item}" (${item.rarity})`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const purchaseCloseBtn = purchaseModal ? purchaseModal.querySelector('.close') : null;
